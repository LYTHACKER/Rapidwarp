...
```bash
git clone --recursive https://github.com/LYTHACKER/Rapidwarp.git rapidwarp-desktop
```
...
On some Linux distributions, Electron will crash with the message The SUID sandbox helper binary was found, but is not configured correctly. Rather than run without sandboxing I'm aborting now. You need to make sure that /home/.../rapidwarp-desktop/node_modules/electron/dist/chrome-sandbox is owned by root and has mode 4755.. Notably we have seen this happen on Debian 10 and earlier and Ubuntu 24.04 and later.
...
Then to package up the final Electron binaries, use either our build script release-automation/build.mjs (see release-automation/README.md) or the electron-builder CLI. Either way the final builds are saved in the dist folder. Here are some examples using the electron-builder CLI directly:
...
Find RapidWarp Desktop's data path by using the list below or by clicking "?" in the top right corner, then "Desktop Settings", then "Open User Data", then opening the highlighted folder, or refer to this list:

· Windows (except Microsoft Store): %APPDATA%/rapidwarp-desktop
· Microsoft Store: Open %LOCALAPPDATA%/Packages, find the folder with the word RapidWarpDesktop in it, then open LocalCache/Roaming/rapidwarp-desktop
· macOS (except Mac App Store): ~/Library/Application Support/rapidwarp-desktop
· Mac App Store: ~/Library/Containers/org.rapidwarp.desktop/Data/Library/Application Support/rapidwarp-desktop (note that the org.rapidwarp.desktop part may appear as RapidWarp in Finder)
· Linux (except Flatpak and Snap): ~/.config/rapidwarp-desktop
· Linux (Flatpak): ~/.var/app/org.rapidwarp.RapidWarp/config/rapidwarp-desktop
